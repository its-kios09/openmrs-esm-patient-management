...
import styles from './add-bed.workspace.scss';
import {
  ButtonSet,
  Button,
  InlineLoading,
  ...
  SelectItem,
  ComboBox,
 Form } from '@carbon/react';
import classNames from 'classnames';
import { z } from 'zod';
import { zodResolver } from '@hookform/resolvers/zod';
import capitalize from 'lodash-es/capitalize';
import { editBed, saveBed, useBedType } from '../../bed-administration/bed-administration.resource';
import { type InitialData } from '../../types/index';
import { useLocationsWithAdmissionTag } from '../../summary/summary.resource';

type AddBedWorkspaceProps = DefaultWorkspaceProps & {
  bed?: InitialData;
  mutateBeds: () => void;
};
...
const bedFormSchema = z.object({
  bedId: z.string().min(5, 'Bed ID must be at least 5 characters').max(255, 'Bed ID must not exceed 255 characters'),
  ...
  occupancyStatus: z.string().refine((value) => value !== '', 'Please select a valid occupancy status'),
  bedType: z.string().refine((value) => value !== '', 'Please select a valid bed type'),
});

type BedFormType = z.infer<typeof bedFormSchema>;

const AddBedWorkspace: React.FC<AddBedWorkspaceProps> = ({
  closeWorkspace,
...
export default AddBedWorkspace;
